...
fn inform_post(payload: Json<InformRequest>) -> String {
  let now = Instant::now();
  let document = documents::document_fabric(
    ...
    payload.paragrafo.clone(), 
    payload.phrase.clone().text.clone(), 
    Vec::new());
  documents::save_document(&document);
  println!("Enlapsed time informing document : {:?}",now.elapsed().as_millis());
  return document.reference.dref;
}
...
pub fn stage() -> rocket::fairing::AdHoc {
  rocket::fairing::AdHoc::on_ignite("DICTUM_server", |rocket| async {
      rocket.mount("/", routes![
          ping,
          phrase_norm_get,
